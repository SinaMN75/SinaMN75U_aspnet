...
public class InquiryService(
	...
	public async Task<UResponse<ZipCodeToAddressDetailResponse?>> ZipCodeToAddressDetail(ZipCodeToAddressDetailParams p, CancellationToken ct) {
		...
		if (inquiryHistory == null || responseBody == null) {
			...
			await walletService.Purchase(new WalletPurchaseParams { AllowOverdraft = true, Tag = TagWalletTxn.ZipCodeToAddressDetail, Token = p.Token, KeyValues = [new KeyValue { Key = ULocalizedConstants.ZipCode, Value = p.ZipCode }] }, ct);
	...
	public async Task<UResponse<VehicleViolationDetailResponse?>> VehicleViolationsDetail(VehicleViolationDetailParams p, CancellationToken ct) {
		...
		if (inquiryHistory == null || responseBody == null) {
			...
			await walletService.Purchase(new WalletPurchaseParams { AllowOverdraft = true, Tag = TagWalletTxn.VehicleViolationsDetail, Token = p.Token, KeyValues = [new KeyValue { Key = ULocalizedConstants.LicencePlate, Value = p.LicencePlate }] }, ct);
	...
	public async Task<UResponse<DrivingLicenceDetailResponse?>> DrivingLicenceDetail(DrivingLicenceDetailParams p, CancellationToken ct) {
		...
		if (inquiryHistory == null || responseBody == null) {
			...
			await walletService.Purchase(new WalletPurchaseParams { AllowOverdraft = true, Tag = TagWalletTxn.DrivingLicenceStatus, Token = p.Token, KeyValues = [new KeyValue { Key = ULocalizedConstants.NationalCode, Value = p.NationalCode }] }, ct);
	...
	public async Task<UResponse<LicencePlateDetailResponse?>> LicencePlateDetail(LicencePlateDetailParams p, CancellationToken ct) {
		...
		if (inquiryHistory == null || responseBody == null) {
			...
			await walletService.Purchase(new WalletPurchaseParams { AllowOverdraft = true, Tag = TagWalletTxn.LicencePlateDetail, Token = p.Token, KeyValues = [new KeyValue { Key = ULocalizedConstants.LicencePlate, Value = p.LicencePlate }] }, ct);
	...
	public async Task<UResponse<DrivingLicenceNegativePointResponse?>> DrivingLicenceNegativePoint(DrivingLicenceNegativePointParams p, CancellationToken ct) {
		...
		if (inquiryHistory == null || responseBody == null) {
			...
			await walletService.Purchase(new WalletPurchaseParams { AllowOverdraft = true, Tag = TagWalletTxn.DrivingLicenceNegativePoint, Token = p.Token, KeyValues = [new KeyValue { Key = ULocalizedConstants.DrivingLicenceNumber, Value = p.DrivingLicenceNumber }] }, ct);
	...
	public async Task<UResponse<FreewayTollsResponse?>> FreewayTolls(FreewayTollsParams p, CancellationToken ct) {
		...
		if (inquiryHistory == null || responseBody == null) {
			...
			await walletService.Purchase(new WalletPurchaseParams { AllowOverdraft = true, Tag = TagWalletTxn.FreewayTolls, Token = p.Token, KeyValues = [new KeyValue { Key = ULocalizedConstants.LicencePlate, Value = p.LicencePlate }] }, ct);
	...
	public async Task<UResponse<IBanToBankAccountDetailResponse?>> IBanToBankAccountDetail(IBanToBankAccountDetailParams p, CancellationToken ct) {
		...
		if (inquiryHistory == null || responseBody == null) {
			...
			await walletService.Purchase(new WalletPurchaseParams { AllowOverdraft = true, Tag = TagWalletTxn.IBanToBankAccountDetail, Token = p.Token, KeyValues = [new KeyValue { Key = ULocalizedConstants.IBan, Value = p.IBan }] }, ct);
	...
	private async Task CreateMobileAndNationalCodeVerificationHistory(string nationalCode, string phoneNumber, bool isVerified, CancellationToken ct) {
		await db.Set<InquiryHistoryEntity>().AddAsync(new InquiryHistoryEntity {
			...
			JsonData = new InquiryHistoryJson(),
	...
	private async Task CreateZipCodeToAddressHistory(string responseBody, ICollection<TagInquiryHistory> tags, string message, ZipCodeToAddressDetailParams p, CancellationToken ct) {
		await db.Set<InquiryHistoryEntity>().AddAsync(new InquiryHistoryEntity {
			...
			CreatorId = Core.App.Users.SystemAdmin.Id,
			CreatedAt = DateTime.UtcNow,
			JsonData = new InquiryHistoryJson { Detail1 = message },
			Tags = tags,
			ZipCode = p.ZipCode,
	...
	// The ItHub token is valid for expires_in seconds; reuse it instead of a token round trip on every inquiry.
	private static GetAccessTokenResponse? _cachedToken;
	...
	protected virtual async Task<GetAccessTokenResponse?> GetAccessToken(CancellationToken ct) {
		if (_cachedToken != null && DateTime.UtcNow < _cachedTokenExpiresAt) return _cachedToken;
		await TokenLock.WaitAsync(ct);
		try {
			if (_cachedToken != null && DateTime.UtcNow < _cachedTokenExpiresAt) return _cachedToken;
			GetAccessTokenResponse? fresh = await RequestAccessToken(ct);
			if (fresh != null) {
				_cachedTokenExpiresAt = DateTime.UtcNow.AddSeconds(Math.Max(0, (fresh.ExpiresIn ?? 300) - 60));
				_cachedToken = fresh;
			}
			return fresh;
		}
