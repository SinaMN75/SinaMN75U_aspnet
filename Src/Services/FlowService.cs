...
public class FlowService(
	...
	public async Task<UResponse> AuthenticationSend(UFlowStepSend p, CancellationToken ct) {
		...
		if (p.Id == FlowStepIds.UserData) {
			...
			if (fatherName == null || (fatherName.Required && fatherName.Value.IsNullOrEmpty())) return new UResponse(Usc.BadRequest, ls.Get("FatherNameRequired"));

			UFlowField? birthdate = p.Fields.FirstOrDefault(x => x.Key == nameof(UserEntity.Birthdate));
			if (birthdate == null || (birthdate.Required && birthdate.Value.IsNullOrEmpty())) return new UResponse(Usc.BadRequest, ls.Get("BirthDateRequired"));
			...
			if (nationalCardFront == null || (nationalCardFront.Required && nationalCardFront.Value.IsNullOrEmpty())) return new UResponse(Usc.BadRequest, ls.Get("NationalCardFrontRequired"));

			UFlowField? nationalCardBack = p.Fields.FirstOrDefault(x => x.Key == nameof(UserEntity.NationalCardBack));
			if (nationalCardBack == null || (nationalCardBack.Required && nationalCardBack.Value.IsNullOrEmpty())) return new UResponse(Usc.BadRequest, ls.Get("NationalCardBackRequired"));

			UFlowField? birthCertificateFirst = p.Fields.FirstOrDefault(x => x.Key == nameof(UserEntity.BirthCertificateFirst));
			if (birthCertificateFirst == null || (birthCertificateFirst.Required && birthCertificateFirst.Value.IsNullOrEmpty())) return new UResponse(Usc.BadRequest, ls.Get("BirthCertificateFirstRequired"));
			...
			if (video == null || (video.Required && video.Value.IsNullOrEmpty())) return new UResponse(Usc.BadRequest, ls.Get("VideoRequired"));
			...
			if (signature == null || (signature.Required && signature.Value.IsNullOrEmpty())) return new UResponse(Usc.BadRequest, ls.Get("SignatureRequired"));

			u.ESignature = ImageCompressor.CompressBase64(signature.Value!);
			u.Tags.Add(TagUser.ESignatureVerified);
		}
		else return new UResponse(Usc.BadRequest, ls.Get("InvalidStep"));
