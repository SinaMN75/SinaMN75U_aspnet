...
public interface IProcessService {
	Task<UResponse<UProcessStepGet?>> AuthenticationGet(BaseParams p, CancellationToken ct);
	Task<UResponse<UProcessStepGet?>> AuthenticationSend(UProcessStepSend p, CancellationToken ct);
}
...
public class ProcessService(
	...
	public async Task<UResponse<UProcessStepGet?>> AuthenticationGet(BaseParams p, CancellationToken ct) {
		...
		if (e.JsonData.FatherName == null || e.Birthdate == null) {
			return new UResponse<UProcessStepGet?>(
				new UProcessStepGet {
					...
					Fields = [
						new UProcessField { Label = "نام پدر", Value = e.JsonData.FatherName, Type = TagFieldType.Text, Required = true, Key = nameof(UserEntity.JsonData.FatherName) },
						new UProcessField { Label = "تاریخ تولد", Value = e.Birthdate?.ToString(), Type = TagFieldType.PersianDate, Required = true, Key = nameof(UserEntity.Birthdate) },
					]
				}
			);
		}
		...
		if (
			...
			return new UResponse<UProcessStepGet?>(
				new UProcessStepGet {
					...
					Fields = [
						new UProcessField { Label = "روی کارت ملی", Value = e.NationalCardFront, Type = TagFieldType.Image, Required = true, Key = nameof(UserEntity.NationalCardFront) },
						new UProcessField { Label = "پشت کارت ملی", Value = e.NationalCardBack, Type = TagFieldType.Image, Required = true, Key = nameof(UserEntity.NationalCardBack) },
						new UProcessField { Label = "صفحه اول شناسنامه", Value = e.BirthCertificateFirst, Type = TagFieldType.Image, Required = true, Key = nameof(UserEntity.BirthCertificateFirst) }
					]
				}
			);
		...
		if (e.VisualAuthentication.IsNullOrEmpty() || !e.Tags.ContainsAny(TagUser.VisualAuthenticationVerified, TagUser.VisualAuthenticationAwaitingVerification))
			return new UResponse<UProcessStepGet?>(
				new UProcessStepGet {
					...
					Fields = [
						new UProcessField { Label = "ویدیو", Value = e.VisualAuthentication, Type = TagFieldType.SelfieVideo, Required = true, Key = nameof(UserEntity.VisualAuthentication) }
					]
				}
			);
		...
		if (e.ESignature.IsNullOrEmpty() || !e.Tags.ContainsAny(TagUser.ESignatureVerified, TagUser.ESignatureAwaitingVerification))
			return new UResponse<UProcessStepGet?>(
				new UProcessStepGet {
					...
					Description = "امضا",
					Fields = [
						new UProcessField { Label = "امضا دیجیتال", Value = e.ESignature, Type = TagFieldType.ESignature, Required = true, Key = nameof(UserEntity.ESignature) }
					]
				}
			);
	...
	public async Task<UResponse<UProcessStepGet?>> AuthenticationSend(UProcessStepSend p, CancellationToken ct) {
		JwtClaimData? userData = ts.ExtractClaims(p.Token);
		if (userData == null) return new UResponse<UProcessStepGet?>(null, Usc.UnAuthorized, ls.Get("AuthorizationRequired"));
