...
public static class FollowRoutes {
	public static void MapFollowRoutes(this IEndpointRouteBuilder app, string tag) {
		...
		r.MapPost("ReadFollowings", async (UserIdParams p, IFollowService s, CancellationToken c) => (await s.ReadFollowings(p, c)).ToResult()).Produces<UResponse<IEnumerable<UserEntity>>>();
