...
public static class FollowRoutes {
	public static void MapFollowRoutes(this IEndpointRouteBuilder app, string tag) {
		...
		r.MapPost("ReadFollowers", async (IdParams p, IFollowService s, CancellationToken c) => (await s.ReadFollowers(p, c)).ToResult()).Cache(1).Produces<UResponse<IEnumerable<UserResponse>>>();
		r.MapPost("ReadFollowedUsers", async (IdParams p, IFollowService s, CancellationToken c) => (await s.ReadFollowedUsers(p, c)).ToResult()).Cache(1).Produces<UResponse<IEnumerable<UserResponse>>>();
		r.MapPost("ReadFollowedProducts", async (IdParams p, IFollowService s, CancellationToken c) => (await s.ReadFollowedProducts(p, c)).ToResult()).Cache(1).Produces<UResponse<IEnumerable<ProductResponse>>>();
		r.MapPost("ReadFollowedCategories", async (IdParams p, IFollowService s, CancellationToken c) => (await s.ReadFollowedCategories(p, c)).ToResult()).Cache(1).Produces<UResponse<IEnumerable<CategoryResponse>>>();
