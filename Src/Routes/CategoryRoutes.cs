...
public static class CategoryRoutes {
	public static void MapCategoryRoutes(this IEndpointRouteBuilder app, string tag) {
		...
		r.MapPost("Create", async (CategoryCreateParams p, ICategoryService s, CancellationToken c) => (await s.Create(p, c)).ToResult()).Produces<UResponse>();
		r.MapPost("BulkCreate", async (List<CategoryCreateParams> p, ICategoryService s, CancellationToken c) => (await s.BulkCreate(p, c)).ToResult()).Produces<UResponse>();
