...
public static class InvoiceRoutes {
	public static void MapInvoiceRoutes(this IEndpointRouteBuilder app, string tag) {
		...
		r.MapPost("Create", async (InvoiceCreateParams d, IInvoiceService s, CancellationToken c) => (await s.Create(d, c)).ToResult()).Produces<UResponse>();
		...
		r.MapPost("Update", async (InvoiceUpdateParams d, IInvoiceService s, CancellationToken c) => (await s.Update(d, c)).ToResult()).Produces<UResponse>();
