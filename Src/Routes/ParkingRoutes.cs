...
public static class ParkingRoutes {
	public static void MapParkingRoutes(this IEndpointRouteBuilder app, string tag) {
		...
		r.MapPost("ReadParking", async (ParkingReadParams p, IParkingService s, CancellationToken c) => (await s.ReadParking(p, c)).ToResult()).Cache(1).Produces<UResponse<IEnumerable<ParkingResponse>>>();
		...
		r.MapPost("ReadParkingReport", async (ParkingReportReadParams p, IParkingService s, CancellationToken c) => (await s.ReadParkingReport(p, c)).ToResult()).Cache(1).Produces<UResponse<IEnumerable<ParkingReportResponse>>>();
