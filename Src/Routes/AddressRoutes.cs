...
public static class AddressRoutes {
	public static void MapAddressRoutes(this IEndpointRouteBuilder app, string tag) {
		...
		r.MapPost("Create", async (AddressCreateParams p, IAddressService s, CancellationToken c) => (await s.Create(p, c)).ToResult()).Produces<UResponse<AddressResponse>>();
		r.MapPost("CreateFromZipCode", async (AddressCreateFromZipCodeParams p, IAddressService s, CancellationToken c) => (await s.CreateFromZipCode(p, c)).ToResult()).Produces<UResponse<AddressResponse>>();
		...
		r.MapPost("Update", async (AddressUpdateParams p, IAddressService s, CancellationToken c) => (await s.Update(p, c)).ToResult()).Produces<UResponse<AddressResponse>>();
