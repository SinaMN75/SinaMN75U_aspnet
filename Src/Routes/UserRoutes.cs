...
public static class UserRoutes {
	public static void MapUserRoutes(this IEndpointRouteBuilder app, string tag) {
		...
		r.MapPost("Create", async (UserCreateParams p, IUserService s, CancellationToken c) => (await s.Create(p, true, c)).ToResult()).Produces<UResponse<UserEntity>>();
		...
		r.MapPost("Update", async (UserUpdateParams d, IUserService s, CancellationToken c) => (await s.Update(d, true, c)).ToResult()).Produces<UResponse<UserEntity>>();
