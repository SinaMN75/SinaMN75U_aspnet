...
public static class VehicleRoutes {
	public static void MapVehicleRoutes(this IEndpointRouteBuilder app, string tag) {
		...
		r.MapPost("Create", async (VehicleCreateParams d, IVehicleService s, CancellationToken c) => (await s.Create(d, c)).ToResult()).Produces<UResponse>();
		...
		r.MapPost("Update", async (VehicleUpdateParams d, IVehicleService s, CancellationToken c) => (await s.Update(d, c)).ToResult()).Produces<UResponse>();
