...
public static class VehicleRoutes {
	public static void MapVehicleRoutes(this IEndpointRouteBuilder app, string tag) {
		...
		r.MapPost("Read", async (VehicleReadParams p, IVehicleService s, CancellationToken c) => (await s.Read(p, c)).ToResult()).Cache(1).Produces<UResponse<IEnumerable<VehicleResponse>>>();
