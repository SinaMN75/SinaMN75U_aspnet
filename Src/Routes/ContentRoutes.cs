...
public static class ContentRoutes {
	public static void MapContentRoutes(this IEndpointRouteBuilder app, string tag) {
		...
		r.MapPost("Create", async (ContentCreateParams d, IContentService s, CancellationToken c) => (await s.Create(d, c)).ToResult()).Produces<UResponse<ContentEntity>>();
		...
		r.MapPost("Update", async (ContentUpdateParams d, IContentService s, CancellationToken c) => (await s.Update(d, c)).ToResult()).Produces<UResponse<ContentEntity>>();
