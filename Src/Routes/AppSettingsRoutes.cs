...
public static class AppSettingsRoutes {
	public static void MapAppSettingsRoutes(this IEndpointRouteBuilder app, string tag) {
		...
		r.MapPost("Read", (BaseParams _) => new UResponse<AppSettingsResponse>(new AppSettingsResponse { ApiCallCosts = Core.App.ApiCallCosts, ChargeInternet = Core.App.ChargeInternet, ChargeInternetTaxPercent = Core.App.ChargeInternetTaxPercent }).ToResult()).Produces<UResponse<AppSettingsResponse>>();
