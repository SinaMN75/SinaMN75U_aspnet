...
public static class TerminalRoutes {
	public static void MapTerminalRoutes(this IEndpointRouteBuilder app, string tag) {
		...
		r.MapPost("CheckAvailability", async (TerminalAssignParams p, ITerminalService s, CancellationToken c) => (await s.CheckAvailability(p, c)).ToResult()).Produces<UResponse<TerminalAvailabilityResponse>>();
