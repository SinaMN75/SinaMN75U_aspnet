...
public static class InquiryRoutes {
	public static void MapInquiryRoutes(this IEndpointRouteBuilder app, string tag) {
		...
		r.MapPost("BillInfo", (BillInfoParams p, IInquiryService s, CancellationToken c) => s.BillInfo(p, c).ToResult()).Produces<UResponse<BillInfoResponse>>();
		...
		r.MapPost("LicencePlateDetail", async (LicencePlateDetailParams p, IInquiryService s, CancellationToken c) => (await s.LicencePlateDetail(p, c)).ToResult()).Produces<UResponse<LicencePlateDetailResponse>>();
