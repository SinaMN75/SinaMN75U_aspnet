...
public static class InquiryRoutes {
	public static void MapInquiryRoutes(this IEndpointRouteBuilder app, string tag) {
		...
		r.MapPost("VehicleViolationDetail", async (VehicleViolationDetailParams p, IInquiryService s, CancellationToken c) => (await s.GetVehicleViolationsDetail(p, c)).ToResult()).Produces<UResponse<VehicleViolationDetailResponse>>();
	}
}