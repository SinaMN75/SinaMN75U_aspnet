...
public static class ContractRoutes {
	public static void MapContractRoutes(this IEndpointRouteBuilder app, string tag) {
		...
		r.MapPost("Create", async (ContractCreateParams d, IContractService s, CancellationToken c) => (await s.Create(d, c)).ToResult()).Produces<UResponse<ContractEntity>>();
		...
		r.MapPost("Update", async (ContractUpdateParams d, IContractService s, CancellationToken c) => (await s.Update(d, c)).ToResult()).Produces<UResponse<ContractEntity>>();
