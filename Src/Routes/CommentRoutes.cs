...
public static class CommentRoutes {
	public static void MapCommentRoutes(this IEndpointRouteBuilder app, string tag) {
		...
		r.MapPost("Create", async (CommentCreateParams d, ICommentService s, CancellationToken c) => (await s.Create(d, c)).ToResult()).Produces<UResponse<CommentEntity>>();
		...
		r.MapPost("Update", async (CommentUpdateParams d, ICommentService s, CancellationToken c) => (await s.Update(d, c)).ToResult()).Produces<UResponse<CommentEntity>>();
