...
public static class CommentRoutes {
	public static void MapCommentRoutes(this IEndpointRouteBuilder app, string tag) {
		...
		r.MapPost("Read", async (CommentReadParams p, ICommentService s, CancellationToken c) => (await s.Read(p, c)).ToResult()).Cache(1).Produces<UResponse<IEnumerable<CommentResponse>>>();
		r.MapPost("ReadById", async (IdParams p, ICommentService s, CancellationToken c) => (await s.ReadById(p, c)).ToResult()).Cache(1).Produces<UResponse<CommentResponse>>();
