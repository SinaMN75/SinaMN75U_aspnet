...
public static class TicketRoutes {
	public static void MapTicketRoutes(this IEndpointRouteBuilder app, string tag) {
		...
		r.MapPost("Read", async (TicketReadParams p, ITicketService s, CancellationToken c) => (await s.Read(p, c)).ToResult()).Cache(1).Produces<UResponse<IEnumerable<TicketResponse>>>();
