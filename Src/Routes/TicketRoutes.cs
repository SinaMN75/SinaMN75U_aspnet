...
public static class TicketRoutes {
	public static void MapTicketRoutes(this IEndpointRouteBuilder app, string tag) {
		...
		r.MapPost("Create", async (TicketCreateParams d, ITicketService s, CancellationToken c) => (await s.Create(d, c)).ToResult()).Produces<UResponse>();
		...
		r.MapPost("Update", async (TicketUpdateParams d, ITicketService s, CancellationToken c) => (await s.Update(d, c)).ToResult()).Produces<UResponse>();
